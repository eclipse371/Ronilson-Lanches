...
class acompanhamentoModel {
  ...
  async gravar() {
    const sql = `INSERT INTO acompanhamentos (nome, preco) VALUES (?, ?)`;
  ...
  async listar() {
    const sql = `SELECT * FROM acompanhamentos`;
    const banco = new Database();
    const lista = [];
    const rows = await banco.ExecutaComando(sql);

    for (let i = 0; i < rows.length; i++) {
      lista.push(new acompanhamentoModel(
        rows[i]["id"],
        rows[i]["nome"],
        rows[i]["preco"]
      ));
  ...
  async obter(id) {
    ...
    if (rows.length > 0) {
      const row = rows[0];
      return new acompanhamentoModel(row["id"], row["nome"], row["preco"]);
    }
  ...
  async alterar() {
    const sql = `UPDATE acompanhamentos SET nome = ?, preco = ? WHERE id = ?`;
  ...
  const sql = `SELECT * FROM acompanhamentos ${whereFiltro} ORDER BY nome`;
  const banco = new Database();
  const rows = await banco.ExecutaComando(sql, valores);
